...
pub const TaskCallback = struct {
    /// The `ctx` (context) is the pointer to e.g., 
...
pub fn TaskScheduler(comptime E: type) type {
    ...
    return struct {
        ...
        /// Polling tasks would execute a task within the main thread.
        ...
        /// ------------------------------------------
        pub fn register(self: *Self, event: Key, cb: TaskCallback) !void {
